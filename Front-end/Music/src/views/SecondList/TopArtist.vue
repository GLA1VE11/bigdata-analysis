...
let artistsData = reactive([]); // 这应该是一个100个元素的数组
...
const displayPage = () => {
  ...
  const pageData = artistsData.slice(startIndex, endIndex);
  // console.log(artistsData, pageData);
  // 配置 ECharts 的选项
  const option = {
    title: {
      text: 'Top50 Artists',
    ...
    xAxis: {
      type: 'category',
      data: pageData.map(item => `Artist ${item.artistId}`),
      axisTick: {
        alignWithLabel: true
      }
    },
    ...
    series: [{
      data: pageData.map(item => item.totalPlays),
      type: 'bar',
      barWidth: '60%',
      label: {
        show: true,
...
const fetchTopArtists = async () => {
  try {
    const response = await request.get('http://localhost:8088/top_artist?top=50');
    artistsData = response.data; // 假设返回的数据是前50名艺术家
    state.totalPages = Math.ceil(artistsData.length / state.pageSize);
    displayPage(); // 显示第一页
  } catch (error) {
